...
using CmdMap = std::map<std::string, TmplCmd>;
static CmdMap cMap =  {
        {"macro",   tcMacro},
        {"pack",    tcPack},
        {"if",      tcIf},
...
struct CmdStack {
    ...
    CmdStack(TmplCmd argCmd, int argIdx, int argLine, int argCol, std::string argValue = {}, int argIdx2 = -1) :
            cmd(argCmd), idx(argIdx), idx2(argIdx2), value(std::move(argValue)), line(argLine), col(argCol)
    {
    }
};
...
public:
    explicit InvalidArgumentIndex(int argIdx) : idx(argIdx)
    {
    }
...
std::string
Template::expandMacro(const MacroInfo& mi, const std::vector<std::string>& args, const std::string& fileName, int line,
                      int col)
{
    std::string rv;
    ...
    while(pos != std::string::npos)
    {
        ...
        if(pos != std::string::npos)
        {
            ...
            else
            {
                ...
                char* end = nullptr;
                int idx = strtol(mi.macroText.c_str() + pos, &end, 10);
                ...
                if(!end)
                {
                    throw TemplateParsingException("macro argument index is invalid", fileName, line, col);
                }
                pos = end - mi.macroText.c_str();
                idx--;
    ...
    switch(expr[pos++])
    {
        case '"':
        {
            ...
            while(expr[pos] != '"')
            {
                if(pos == expr.length())
                {
                    throw BoolExprParsingExpr("Unexpected end of string value", spos);
                }
                if(expr[pos] == '\\')
                {
                    ++pos;
                    ...
                    switch(expr[pos])
                    {
                        ...
                        {
                            char* end = nullptr;
                            const char* ptr = expr.c_str() + pos + 1;
                            int hex = strtol(ptr, &end, 16);
                            if(!end || end == ptr || *end=='\0')
                            {
                                throw BoolExprParsingExpr("Invalid hex value", pos);
                            }
                            if(hex > 255)
                            {
                                throw BoolExprParsingExpr("Hex value is out of range", pos);
                            }
                            pos += 1 + end - ptr;
                            val += static_cast<char>(hex);
                            break;
                        }
                        default:
                            throw BoolExprParsingExpr("Unexpected escape symbol", pos);
                    }
                    ++pos;
                    continue;
                }
...
std::string::size_type Template::parseBool(const std::string& expr, BoolTree& bt, std::string::size_type pos, int prio)
{
    std::string val;
    BoolTerm t;
    ...
    while((t = getNextTerm(expr, (oldPos = pos, pos), val)) != btEnd)
    {
        ...
        else if(t == btNot)
        {
            ...
            {
                bt.bop = bopNot;
                bt.left = std::make_unique<BoolTree>(tmp);
            }
            continue;
        }
        int newPrio;
        ...
        {
            bt.left = std::make_unique<BoolTree>(bt);
            bt.bop = bop;
            bt.right = std::make_unique<BoolTree>();
...
void Template::Parse(FileReader& fr)
{
        ...
        if(c == '$')
        {
            ...
            switch(it->second)
            {
                ...
                case tcIf:
                {
                    ...
                    {
                        stack.emplace_back(tcIf, ops.size(), line, col);
                        Op op;
                        op.op = opIf;
                        line = fr.line;
                        col = fr.col;
                ...
                case tcIfndef:
                {
                    if(cmdEnd)
                    {
                        ...
                        {
                            throw TemplateParsingException("Unexpected end of ifdef/ifndef command", fr.fileName, line,
                                    col);
                        }
                        ops[stack.back().idx].jidx = ops.size();
                        stack.pop_back();
                    ...
                    {
                        stack.emplace_back(it->second, ops.size(), line, col);
                        Op op;
                ...
                case tcForEach:
                {
                    ...
                    {
                        stack.emplace_back(tcForEach, ops.size(), line, col);
                        Op op;
                        op.op = opLoop;
                        op.line = line;
                        op.col = col;
                ...
                case tcSelect:
                {
                    if(cmdEnd)
                    {
                        ...
                        while(!stack.empty() && (stack.back().cmd == tcCase || stack.back().cmd == tcDefault))
                        {
                            ...
                            {
                                throw TemplateParsingException("Celect cannot have two default cases", fr.fileName,
                                        line, col);
                            }
                    ...
                    {
                        stack.emplace_back(tcSelect, ops.size(), line, col);
                        Op op;
                        op.op = opSelect;
                        op.line = line;
                        op.col = col;
                ...
                case tcSetBool:
                {
                    ...
                    {
                        throw TemplateParsingException("Value in setbool can only be true or false", fr.fileName, line,
                                col);
                    }
                ...
                case tcExpand:
                {
                    ...
                    auto mit = macroMap.find(macroName);
                    if(mit == macroMap.end())
                    {
                    ...
                    try
                    {
                        ...
                        macroFr.source.insert(macroFr.source.begin(), macroTxt.c_str(),
                                macroTxt.c_str() + macroTxt.length());
                ...
                case tcComment:
                    for(;;)
                    {
                        ...
                        if(c == '$')
                        {
                            ...
                            std::string opname2 = getContent(fr, "$", c);
                            CmdMap::const_iterator it2 = cMap.find(opname2);
                            if(it2 != cMap.end())
                            {
                                if(it2->second == tcComment)
                                {
                                    break;
                                }
                            }
    ...
    if(!stack.empty())
    {
        ...
        for(auto& p:cMap)
        {
            if(stack.back().cmd == p.second)
            {
                cmd = p.first;
                break;
            }
        }
        throw TemplateParsingException("Command " + cmd + " not ended at the end of template", fr.fileName,
                stack.back().line, stack.back().col);
    }
}
    ...
    for(size_t i = 0; i < ops.size(); i++)
    {
        ...
        switch(ops[i].op)
        {
            ...
            case opPackEnd:
                ...
                break;
            case opSetBool:
                printf("setbool:%s:%s\n", ops[i].value.c_str(), ops[i].boolSetValue ? "true" : "false");
                break;
            case opSetVar:
            {
